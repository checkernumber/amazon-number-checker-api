# Amazon Number Checker API — Bulk Verification Examples

The **Amazon Number Checker API** is a bulk, asynchronous, file-based API used to check Amazon account registration by phone number or email address. Upload an input file, poll the returned task ID, then download the exported results. This repository contains runnable examples in Python, Node.js, Go, Java, C#, PHP, browser JavaScript, and Shell.

- **Base URL:** `https://api.checknumber.ai`
- **Auth:** `X-API-Key` request header
- **Full API docs:** https://docs.checknumber.ai/amazon-bulk-number-checker
...
- [How do I check if a phone number is registered on Amazon?](#how-do-i-check-if-a-phone-number-is-registered-on-amazon)
- [How do I process Amazon checks in bulk?](#how-do-i-process-amazon-checks-in-bulk)
...
- [How do I check if an email is registered on Amazon?](#how-do-i-check-if-an-email-is-registered-on-amazon)
- [What does the Amazon number checker API cost?](#what-does-the-amazon-number-checker-api-cost)
- [Amazon number checker API vs alternatives](#amazon-number-checker-api-vs-alternatives)
- [Code examples](#code-examples)
- [Status codes](#status-codes)
- [FAQ](#faq)
- [Support & legal](#support--legal)

## How do I check if a phone number is registered on Amazon?

Send a `POST` request to `https://api.checknumber.ai/v1/tasks` with the API key, `numbers.txt`, and `task_type=amazon`. The response contains a `task_id` used to retrieve task status and the final `result_url`.

```bash
curl --location 'https://api.checknumber.ai/v1/tasks' \
  --header 'X-API-Key: YOUR_API_KEY' \
  --form 'file=@"./numbers.txt"' \
  --form 'task_type="amazon"'
```

Each line in `numbers.txt` is one phone number in E.164 format, such as `+14155552671`.

## How do I process Amazon checks in bulk?
...
| `task_type` | string | One of: `amazon`, `amazon_email`. |
...
**Main task result fields — `task_type=amazon`**
...
## How do I check if an email is registered on Amazon?

Checks Amazon registration using a file containing one email address per line. Use `task_type=amazon_email`; the submit, poll, and download workflow is otherwise unchanged.
...
Result fields (copied from the [canonical documentation](https://docs.checknumber.ai/amazon-email-checker)):
...
## What does the Amazon number checker API cost?
...
## Amazon number checker API vs alternatives

| Capability | CheckNumber API | Generic carrier lookup | Manual checking |
| --- | --- | --- | --- |
| Checks the documented Amazon signal | Yes | No | May be possible |
...
- `task_type=amazon` — [Amazon Phone Number Checker](https://docs.checknumber.ai/amazon-bulk-number-checker); Checks Amazon registration using a file of E.164 phone numbers. Input: numbers.txt.
- `task_type=amazon_email` — [Amazon Email Checker](https://docs.checknumber.ai/amazon-email-checker); Checks Amazon registration using a file containing one email address per line. Input: emails.txt.
...
- **Documentation:** https://docs.checknumber.ai/amazon-bulk-number-checker
- **Dashboard and API keys:** https://platform.checknumber.ai
- **License:** [MIT](./LICENSE) for the sample code

Process only data you are authorized to use and comply with applicable privacy laws and platform terms. CheckNumber is not affiliated with or endorsed by Amazon.

---

*Last updated: 2026-07-13 · Maintained by CheckNumber. Canonical docs: https://docs.checknumber.ai/amazon-bulk-number-checker*
